...
- Введите ID команды, чтобы выполнить действие (например, 0 для импорта или 1 для добавления кошелька)
...
>
> В каталоге [Scripts](Scripts) находится `.py` python-скрипт для генерации `.json` файла с тестовыми данными.
> При запуске скрипт потребует указать путь до `.json` файла, который будет перезаписан новосгенерированными данными.
>
> При использовании команды для испорта данных, в начале будет запущено окно выбора `.json` файла с данными
> (если тут произойдёт какая-либо ошибка, то будет предложено ввести путь текстом в консоль),
> далее будет произведена попытка загрузить данные с проверкой их валидности под условия задачи.
